...
def llm_calculate_reimbursement(data: dict, baseline: float, itemized_items: list = None, 
                                rag_context: str = "", fraud_result: dict = None, 
                                shap_context: str = "") -> dict:
...
- Fraud Score: {fraud_result.get('fraud_score', 0):.4f}
...
{{
  ...
  "flagged_items": [],
  "policy_type": "standard" | "high_risk",
  "confidence": 0.XX
}}"""
...
def _fallback_decision(data: dict, baseline: float) -> dict:
    ...
    if pre_auth == "Yes":
        return {
            ...
            "explanation": f"Pre-authorization confirmed. Baseline ${baseline:,.2f} × 85% = ${reimburse:,.2f}",
            "flagged_items": [],
    ...
    if amount > PRE_AUTH_THRESHOLD:
        return {
            ...
            "explanation": f"Claim denied: Amount exceeds ${PRE_AUTH_THRESHOLD:,.0f} without pre-authorization.",
            "flagged_items": [],
            "policy_type": "standard",
            "confidence": 0.90
        }
    ...
    return {
        ...
        "explanation": f"Approved at reduced rate. Baseline ${baseline:,.2f} × 70% = ${reduced:,.2f}",
        "flagged_items": [],
        "policy_type": "standard",
        "confidence": 0.75
    }
...
def get_decision(parsed: dict, rag_context: str = "", itemized_items: list = None, 
                 fraud_result: dict = None, shap_context: str = "") -> dict:
    ...
    llm_result = llm_calculate_reimbursement(
        data, baseline, itemized_items, rag_context, fraud_result, shap_context
    )
...
def process_claim(file_bytes: bytes = None, form_data: dict = None, db_knowledge=None, db_claims=None) -> dict:
    ...
    fraud_result = predict_fraud(extracted_data)
    shap_contributions = explain_decision(extracted_data, fraud_result)
    ...
    data = decision_result.pop("_data", None) or _build_data(parsed, safe_float(parsed.get("claimamount") or 0))
