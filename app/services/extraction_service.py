...
def extract_prescription_drugs(raw_text: str, diagnosis_code: str = None) -> list:
    ...
    bills = extract_itemized_bill(raw_text, diagnosis_code)
...
def rule_extract(text: str) -> tuple:
    ...
    data["Employment"] = data["PatientEmploymentStatus"]
    ...
    for pat in [
        ...
        m = re.search(pat, t)
        if m:
            data["DateOfService"] = m.group(1).strip()
            break
    ...
    m = re.search(r'(?:insurance|payer|coverage)\s*[:\-]?\s*([A-Za-z0-9\s\&\.\-,]+?)(?=\s*(?:policy|amount|patient|$))', t)
    if m:
